...
class CLWrapperClass_dotproduct_precompiled_MapKernel_mul_arity_2
{
public:
...
__kernel void dotproduct_precompiled_MapKernel_mul_arity_2(__global float *a, __global float *b,  __global float* output, size_t w, size_t n, size_t base)
{
	...
	while (i < n)
	{
		...
		for (skepu2::backend::Device_CL *device : skepu2::backend::Environment<int>::getInstance()->m_devices_CL)
		{
			...
			cl_kernel kernel = clCreateKernel(program, "dotproduct_precompiled_MapKernel_mul_arity_2", &err);
			CL_CHECK_ERROR(err, "Error creating map kernel '" << "dotproduct_precompiled_MapKernel_mul_arity_2" << "'");
