...
class CLWrapperClass_dotproduct_precompiled_MapReduceKernel_mul_add_arity_2
{
public:
...
__kernel void dotproduct_precompiled_MapReduceKernel_mul_add_arity_2(__global float *a, __global float *b,  __global float* output, size_t w, size_t n, size_t base, __local float* sdata)
{
...
__kernel void dotproduct_precompiled_MapReduceKernel_mul_add_arity_2_ReduceOnly(__global float* input, __global float* output, size_t n, __local float* sdata)
{
	size_t blockSize = get_local_size(0);
	size_t tid = get_local_id(0);
	...
	if (tid == 0)
	{
		...
		for (skepu2::backend::Device_CL *device : skepu2::backend::Environment<int>::getInstance()->m_devices_CL)
		{
			...
			cl_kernel kernel_mapreduce = clCreateKernel(program, "dotproduct_precompiled_MapReduceKernel_mul_add_arity_2", &err);
			CL_CHECK_ERROR(err, "Error creating MapReduce kernel '" << "dotproduct_precompiled_MapReduceKernel_mul_add_arity_2" << "'");
			
			cl_kernel kernel_reduce = clCreateKernel(program, "dotproduct_precompiled_MapReduceKernel_mul_add_arity_2_ReduceOnly", &err);
			CL_CHECK_ERROR(err, "Error creating MapReduce kernel '" << "dotproduct_precompiled_MapReduceKernel_mul_add_arity_2" << "'");
