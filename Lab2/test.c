...
/** add by us **/
int pool [NB_THREADS][MAX_PUSH_POP/NB_THREADS];
int pool_index [NB_THREADS];
/** **/
    ...
    for (i = 0; i < MAX_PUSH_POP / NB_THREADS; i++)
      {
        ...
        popValue=stack_pop(stack);
        pool[id][pool_index[id]]=popValue;
        pool_index[id] = pool_index[id] +1;
  ...
  for (i = 0; i < MAX_PUSH_POP / NB_THREADS; i++)
    {
        ...
        /**add by us **/
        stack_push(stack, (id+1)*1000000+i);
        /** **/
    }
  clock_gettime(CLOCK_MONOTONIC, &t_stop[args->id]);
