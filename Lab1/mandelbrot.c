...
#endif
// Compiled only if LOADBALANCE = 1 naive
#if LOADBALANCE == 1
	
	int id = args->id;
	int h = parameters->height;
	
	parameters->begin_h = id * h / NB_THREADS;
	parameters->end_h = (id==NB_THREADS-1) ? h : ((id+1) * h / NB_THREADS);
...
#endif
// Compiled only if LOADBALANCE = 2 sliced
#if LOADBALANCE == 2
	
	int id = args->id;
	int h = parameters->height;
	int den = NB_THREADS * NB_THREADS;
	int index =0;
	int n=0;
	
	for (index=0; index<den/NB_THREADS; index++){
		n=id + NB_THREADS*index;
		parameters->begin_h = n * h / den;
		
		parameters->end_h = (n+1) * h / den;
