...
// Filter kernel (simple box filter)
	...
	//out[(y*imagesizex+x)*3+0] = image[(y*imagesizex+x)*3+0];
	//out[(y*imagesizex+x)*3+1] = image[(y*imagesizex+x)*3+1];
	//out[(y*imagesizex+x)*3+2] = image[(y*imagesizex+x)*3+2];
	}
}
...
// You can save the result to a file like this:
	writeppm("solecito.ppm", imagesizey, imagesizex, pixels);
