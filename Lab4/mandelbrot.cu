...
__device__
int mandelbrot( int x, int y, MYFLOAT scale, MYFLOAT ox, MYFLOAT oy, MYFLOAT zoom, int max)
{
    MYFLOAT jx = scale * (MYFLOAT)(DIM/2 - x + ox/scale)/(DIM/2);
    MYFLOAT jy = scale * (MYFLOAT)(DIM/2 - y + oy/scale)/(DIM/2);
...
__global__
void computeFractalCuda(unsigned char* img, MYFLOAT scale, MYFLOAT ox, MYFLOAT oy, MYFLOAT zoom, int max){
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	int idy = blockIdx.y * blockDim.y + threadIdx.y;
	int idOffset = idx + idy * blockDim.x * gridDim.x;


	int value = mandelbrot(idx, idy, scale, ox, oy, zoom, max);

	// Colorize it
		    int red = 255 * value/max;
		    if (red > 255) red = 255 - red;
		    int green = 255 * value*4/max;
		    if (green > 255) green = 255 - green;
		    int blue = 255 * value*20/max;
		    if (blue > 255) blue = 255 - blue;
		    
		    img[idOffset*4 + 0] = red;
		    img[idOffset*4 + 1] = green;
		    img[idOffset*4 + 2] = blue;
		    
		    img[idOffset*4 + 3] = 255;
}
...
void Draw()
{
	...
	dim3 dimGrid(DIM/BLOCKSIZE,DIM/BLOCKSIZE);
	dim3 dimBlock(BLOCKSIZE,BLOCKSIZE);
	cudaEventRecord(start,0);
	computeFractalCuda<<<dimGrid, dimBlock>>>(img, scale, offsetx, offsety, zoom, maxiter);
	cudaThreadSynchronize();
	cudaEventRecord(stop,0);
	cudaMemcpy( pixels, img, size, cudaMemcpyDeviceToHost );
